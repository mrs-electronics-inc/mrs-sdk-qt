...
However, QMake does not have an equivalent feature, so QMake-based projects must bootstrap from the `.pro` file using the `QMAKE_TOOLCHAIN_FILE` variable defined in the Qt Creator kit settings:
