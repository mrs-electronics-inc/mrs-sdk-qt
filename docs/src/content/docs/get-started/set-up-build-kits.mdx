...
2. There will be a few tabs across to the top of the window. Go to **Qt Versions**.
...
4. Go to **Compilers**. Verify the GCC/G++ compilers were auto-detected. There may be multiple compiler options on your system:
    ...
    ![Qt Creator compiler config for desktop Qt5](../../../assets/build-kit-setup/desktop-qt5/compiler.png)

    Look for paths like `/bin/gcc` or `/usr/lib/ccache/gcc`.

5. Go to **Debuggers**. Verify a system GDB debugger was auto-detected:

    ![Qt Creator debugger config for desktop Qt5](../../../assets/build-kit-setup/desktop-qt5/debugger.png)
...
7. Verify a system CMake tool was auto-detected:
    ...
    Look for paths like `/usr/bin/cmake`. Run `apt install cmake` to install CMake if necessary.

8. Now that all the kit's components are verified, it's time to create the kit! Navigate back to **Kits**.
9. The Qt 5.15 desktop kit should've been auto-detected. Edit the configuration as needed to match this:

    ![Qt Creator debugger config for desktop Qt5](../../../assets/build-kit-setup/desktop-qt5/kit-full.png)
...
10. Next, you'll need to add some environment variables to configure the SDK. Click **Edit Build Environment** and add the following, inserting the path to your MRS SDK installation:
    ...
    ![Qt Creator build environment config for desktop Qt5](../../../assets/build-kit-setup/desktop-qt5/kit-build-env.png)
...
11. Finally, you need to verify CMake is configured for the SDK properly. Click the **Change...** button next to **CMake Configuration** and add the following, inserting the path to your MRS SDK installation:
    ...
    ![Qt Creator CMake config for desktop Qt5](../../../assets/build-kit-setup/desktop-qt5/kit-cmake-config.png)
