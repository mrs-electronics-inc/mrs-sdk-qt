...
if("${MRS_SDK_QT_QT_MAJOR_VERSION}" STREQUAL "5")
    ...
    if(Qt5Core_VERSION)
        ...
        if(MRS_SDK_QT_OS_DESKTOP)
            ...
            math(EXPR _minor "${CMAKE_MATCH_2} + 1")
            set(_upper_bound "${CMAKE_MATCH_1}.${_minor}.0")
            if(("${Qt5Core_VERSION}" VERSION_GREATER_EQUAL "${MRS_SDK_QT_EXPECTED_QT_VERSION}.0")
               AND ("${Qt5Core_VERSION}" VERSION_LESS "${_upper_bound}")
               AND ("${MRS_SDK_QT_QT_MAJOR_VERSION}" STREQUAL "5"))
                message(NOTICE "Qt version: ${Qt5Core_VERSION}")
            else()
...
elseif("${MRS_SDK_QT_QT_MAJOR_VERSION}" STREQUAL "6")
    ...
    if(Qt6Core_VERSION)
        ...
        if(MRS_SDK_QT_OS_DESKTOP)
            ...
            math(EXPR _minor "${CMAKE_MATCH_2} + 1")
            set(_upper_bound "${CMAKE_MATCH_1}.${_minor}.0")
            if(("${Qt6Core_VERSION}" VERSION_GREATER_EQUAL "${MRS_SDK_QT_EXPECTED_QT_VERSION}.0")
               AND ("${Qt6Core_VERSION}" VERSION_LESS "${_upper_bound}")
               AND ("${MRS_SDK_QT_QT_MAJOR_VERSION}" STREQUAL "6"))
                message(NOTICE "Qt version: ${Qt6Core_VERSION}")
            else()
...
if(NOT _mrs_sdk_qt_qt_core_target)
